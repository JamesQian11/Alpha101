...
for k in range(div):
    ...
    corr_list.append(
        [np.array(corr_1).mean(), np.array(corr_2).mean(), np.array(corr_3).mean(), np.array(corr_4).mean()])
...
# 各组累计IC时序图
y1, y2, y3, y4, y5, y6, y7, y8, y9 = ic_cul[0], ic_cul[1], ic_cul[2], ic_cul[3], ic_cul[4], ic_cul[-4], ic_cul[-3], \
                                     ic_cul[-2], ic_cul[-1]
...
plt.plot(d, y1, label='alpha1');
plt.plot(d, y2, label='alpha2');
plt.plot(d, y3, label='alpha3');
plt.plot(d, y4, label='alpha4')
plt.plot(d, y5, label='alpha5');
plt.plot(d, y6, label='对数总市值');
plt.plot(d, y7, label='20日收益率')
plt.plot(d, y8, label='20日换手率');
plt.plot(d, y9, label='20日波动率')
plt.legend()
...
# 分层各组收益时序图，探究单个因子
n = 0
y1, y2, y3, y4, y5, y6, y7, y8, y9 = df[n][1], df[n][2], df[n][3], df[n][4], df[n][5], df[n][6], df[n][7], df[n][8], \
                                     df[n][9]
...
plt.plot(d, y4, label='group4')
plt.plot(d, y5, label='group5');
plt.plot(d, y6, label='group6');
plt.plot(d, y7, label='group7');
plt.plot(d, y8, label='group8')
plt.plot(d, y9, label='group9');
plt.plot(d, y10, label='group10');
plt.plot(d, y11, label='top-bottom')
plt.legend()
...
# 多空组收益净值变化，探究多个因子
y1, y2, y3, y4, y5, y6, y7, y8, y9 = df[0][-1], df[1][-1], df[2][-1], df[3][-1], df[4][-1], df[-4][-1], df[-3][-1], \
                                     df[-2][-1], df[-1][-1]
bench = dff[0][0]  # 基准线
d = list(next_ret.index.levels[0])
plt.subplots(figsize=(15, 5))  # 图的长宽设置
