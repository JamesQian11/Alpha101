...
def alpha47(volume, close, high, vwap):
    adv20 = sma(volume, 20)
    alpha = ((rank((1 / close)) * volume) / adv20) * ((high * rank((high - close))) / sma(high, 5)) - rank(
        (vwap - delay(vwap, 5)))
    return alpha.fillna(value=0)
...
def alpha82(Open, volume, ind):
    ...
    r2 = ts_rank(
        decay_linear(correlation(IndNeutralize(volume, ind), ((Open * 0.634196) + (Open * (1 - 0.634196))), 17), 7), 13)
    alpha = r1
    alpha[r1 > r2] = r2
    return -1 * alpha.fillna(value=0)
